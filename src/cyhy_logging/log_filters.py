"""Custom logging filters for the CyHy logging framework."""
...
class RedactPasswordFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> logging.LogRecord | Literal[True]:
        ...
        # Redact passwords
        redacted_message = self.redact_passwords(formatted_message)

        # Only create a new record if the message was actually redacted
        if formatted_message == redacted_message:
